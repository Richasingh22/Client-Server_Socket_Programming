...
int serverSelection()
{
	// Mirror Server = 0
	// Primary Server = 1
	// if totalClients <= 8, then primary server handles first 4 clients and mirror server handles next 4 clients, and then after totalClients > 8, primary and mirror servers handle alternate clients
...
int main()
{
	...
	while (1)
	{
		...
		// call the server selection function to decide which server will handle the client
		int selectedServer = serverSelection();
		if (selectedServer)
		{
			// Primary Server
			noOfPrimaryServerClients++;
			// send the client a P to indicate that it need to stay connected to Primary Server
			write(newSocket, "P", 1);
			totalClients++;
			printf("\n");
		}
		else
		{
			...
			// send the client a M to indicate that it need to be redirected to Mirror Server
			write(newSocket, "M", 1);
			close(newSocket);
			totalClients++;
			printf(" - Redirected to Mirror Server\n");
			// skip handling the client as it will be handled by the Mirror Server
			continue;
		}
